...
static int private_build_level(LIMdlBuilder*    self,
							   LIMdlBuilderLod* lod,
							   LIMdlEdge**      edges_left,
							   LIMdlEdge**      edges_sort,
							   float            target_cost);

static float private_calculate_collapse_cost(LIMdlBuilder*    self,
											 LIMdlBuilderLod* lod,
											 LIMdlEdge*       edge);

static int private_collapse_edge(LIMdlBuilder*    self,
								 LIMdlBuilderLod* lod,
								 LIMdlEdge*       edge,
								 LIMdlEdge**      edges_left);

static LIMdlEdge* private_extract_edges(LIMdlBuilder*    self,
									    LIMdlBuilderLod* lod,
										int              index_count,
										int*             edge_count);

static LIMdlEdge* private_extract_sliding_edges(LIMdlBuilder*    self,
												LIMdlBuilderLod* lod,
												int              index_count,
												LIMdlEdge*       edges,
												int              edge_count);
 ...
 */
int limdl_builder_calculate_lod(LIMdlBuilder* self,
								int           levels,
								float         factor)
{
	int i;
...
static int private_compare_edges(const void* a,
								 const void* b)
{
	LIMdlEdge* const* A = a;
	LIMdlEdge* const* B = b;
...
static float private_calculate_collapse_cost_attr(LIMdlBuilder* 	self,
												  LIMdlBuilderLod*  lod,
												  LIMdlEdge* 		edge)
{
	LIMdlVertex* v1 = self->model->vertices.array + edge->i1;
	LIMdlVertex* v2 = self->model->vertices.array + edge->i2;
	vec3 d_nml = limat_vector_subtract (v1->normal, v2->normal);

	float d_tex[2];
	float d_col[4];

/*
	float d_tex[2] = { v1->texcoord[0] - v2->texcoord[0], 
					   v1->texcoord[1] - v2->texcoord[1] };

	float d_col[4] = { v1->color[0] - v2->color[0], 
					   v1->color[1] - v2->color[1],
	                   v1->color[2] - v2->color[2], 
	                   v1->color[3] - v2->color[3] };
*/



	d_tex[0] = v1->texcoord[0] - v2->texcoord[0];
	d_tex[1] = v1->texcoord[1] - v2->texcoord[1];

	d_col[0] = v1->color[0] - v2->color[0];
	d_col[1] = v1->color[1] - v2->color[1];
	d_col[2] = v1->color[2] - v2->color[2];
	d_col[3] = v1->color[3] - v2->color[3];
...
static float private_calculate_collapse_cost_geom(LIMdlBuilder*    self,
												  LIMdlBuilderLod* lod,
												  LIMdlEdge*       edge)
{
	LIMdlVertex* v1 = self->model->vertices.array + edge->i1;
	LIMdlVertex* v2 = self->model->vertices.array + edge->i2;
	vec3 d_vtx = limat_vector_subtract (v1->coord, v2->coord);
