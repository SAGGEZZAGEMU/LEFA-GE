...
static inline void limat_aabb_init (LIMatAabb* self)
{
	...
	vec3*         result)
{
	...
	vec3 buffer[18];
	vec3* tmpbuf;
	vec3* prevbuf = buffer + 0;
	vec3* currbuf = buffer + 9;
	LIMatPlane p[6];
...
*/
	...
	p[0].x = 1;  p[0].y = 0;  p[0].z = 0;  p[0].w = self->min.x;  // Left
	p[1].x = -1; p[1].y = 0;  p[1].z = 0;  p[1].w = -self->max.x; // Right
	p[2].x = 0;  p[2].y = 1;  p[2].z = 0;  p[2].w = self->min.y;  // Bottom
	p[3].x = 0;  p[3].y = -1; p[3].z = 0;  p[3].w = -self->max.y; // Top
	p[4].x = 0;  p[4].y = 0;  p[4].z = 1;  p[4].w = self->min.z;  // Front
	p[5].x = 0;  p[5].y = 0;  p[5].z = -1; p[5].w = -self->max.z; // Back
	...
	memcpy (result, prevbuf, prevlen * sizeof (vec3));
	return prevlen;
}
