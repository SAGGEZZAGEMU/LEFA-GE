...
#ifdef HAVE_INOTIFY
static int private_process(LISysNotify* self);

static int private_read (LISysNotify* self);
#endif

/*****************************************************************************/

LISysNotify* lisys_notify_new ()
{
#ifdef HAVE_INOTIFY
	LISysNotify* self;
...
void lisys_notify_free(LISysNotify* self)
{
	close (self->fd);
	free (self->buffer.buffer);
	free (self);
...
int lisys_notify_add(LISysNotify*     self,
                  	 const char*      path,
