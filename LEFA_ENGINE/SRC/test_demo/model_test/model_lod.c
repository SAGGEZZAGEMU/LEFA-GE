...
int limdl_lod_init_copy(LIMdlLod* self,
						LIMdlLod* lod)
{
	int i;
...
void limdl_lod_free(LIMdlLod* self)
{
	lisys_free (self->face_groups.array);
	lisys_free (self->indices.array);
}

int limdl_lod_read(LIMdlLod*    self,
				   LIArcReader* reader)
{
	int pos;
	uint32_t i;
...
int limdl_lod_read_old(LIMdlLod*    self,
					   LIArcReader* reader)
{
	uint32_t i;
	uint32_t j;
...
int limdl_lod_write(const LIMdlLod* self,
					LIArcWriter*    writer)
{
	int i;
	LIMdlFaces* group;
...
int limdl_lod_get_memory(const LIMdlLod* self)
{
