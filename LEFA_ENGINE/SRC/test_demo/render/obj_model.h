...
#include"3rdparty/tol/objloader.h"
...
static void CalcNormal(float N[3], 
                       float v0[3], 
                       float v1[3], 
                       float v2[3]) 
{
...
static const char* mmap_file(size_t* len, const char* filename) 
{
    ...
    // Código específico para Windows (32 ou 64 bits)
...
static int LoadObjAndConvert(float bmin[3], 
                             float bmax[3],
                             const char* filename) 
{
...
typedef struct 
{
    GLuint vbo;         // Vertex Buffer Object para vértices
    GLuint texVBO;      // Vertex Buffer Object para coordenadas de textura
    GLuint texture;     // ID da textura (pode ser configurado depois)
    size_t numTriangles; // Número de triângulos
} ObjModel;
...
int LoadOBJ(const char* filename, ObjModel* model) 
{
    ...
    ret = tinyobj_parse_obj(&attrib, &shapes, &num_shapes, &materials, &num_materials, data, data_len, flags);
    if (ret != TINYOBJ_SUCCESS) {
        fprintf(stderr, "Erro ao parsear modelo: %s\n", filename);
        return 0;
    }

    num_triangles = attrib.num_faces / 3;
    vertices = malloc(sizeof(float) * num_triangles * 9);
    texcoords = malloc(sizeof(float) * num_triangles * 6);
    if (!vertices || !texcoords) {
        fprintf(stderr, "Erro ao alocar memória para vértices ou coordenadas de textura.\n");
        free(vertices);
        free(texcoords);
        return 0;
    }
    ...
    for (i = 0; i < attrib.num_faces; i++) {
        ...
        vertices[i * 3 + 0] = attrib.vertices[idx.v_idx * 3 + 0];
        vertices[i * 3 + 1] = attrib.vertices[idx.v_idx * 3 + 1];
        vertices[i * 3 + 2] = attrib.vertices[idx.v_idx * 3 + 2];

        if (idx.vt_idx >= 0) {
            texcoords[i * 2 + 0] = attrib.texcoords[idx.vt_idx * 2 + 0];
            texcoords[i * 2 + 1] = attrib.texcoords[idx.vt_idx * 2 + 1];
        }
    }

    // Cria o VBO para vértices
    glGenBuffers(1, &model->vbo);
    glBindBuffer(GL_ARRAY_BUFFER, model->vbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * num_triangles * 9, vertices, GL_STATIC_DRAW);

    // Cria o VBO para coordenadas de textura
    glGenBuffers(1, &model->texVBO);
    glBindBuffer(GL_ARRAY_BUFFER, model->texVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * num_triangles * 6, texcoords, GL_STATIC_DRAW);

    model->numTriangles = num_triangles;
    model->texture = 0; // Nenhuma textura associada inicialmente
...
void LinkTextureToModel(ObjModel* model, 
                        GLuint textureID) 
{
    if(model) 
    {
...
void RenderModel(const ObjModel* model) 
{
    if (model->vbo == 0) return; // Verifica se o modelo foi carregado
    ...
    if (model->texVBO) glDisableClientState(GL_TEXTURE_COORD_ARRAY);
