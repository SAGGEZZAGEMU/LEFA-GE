...
int limdl_shape_init_copy (LIMdlShape*       self,
							const LIMdlShape* shape)
{
	...
	for (i = 0 ; i < self->parts.count ; i++)
	{
		...
		if (srcpart->vertices.count)
		{
			...
			dstpart->vertices.array = lisys_calloc (3 * srcpart->vertices.count, sizeof (vec3));
...
void limdl_shape_clear (LIMdlShape*  self)
{
	int i;
	...
	LIArcReader* reader)
{
	...
	for (i = 0 ; i < self->parts.count ; i++)
	{
		...
		if (tmp)
		{
			...
			part->vertices.array = lisys_calloc (3 * tmp, sizeof (vec3));
 ...
 */
void limdl_shape_get_center (const LIMdlShape*  self,
							 vec3*      		result)
{
	*result = self->center;
}
