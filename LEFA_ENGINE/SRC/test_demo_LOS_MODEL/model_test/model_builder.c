...
static void private_weld_normals(LIMdlBuilder* self,
								 int vertex_offset);

static int private_faces_init(LIMdlBuilderFaces* self,
							  LIMdlFaces*        src,
						      LIMdlLod*          srclod);

static int private_faces_copy(LIMdlBuilderFaces* self,
							  LIMdlBuilderFaces* src);

static void private_faces_clear(LIMdlBuilderFaces* self);

static int private_lod_init(LIMdlBuilderLod* self,
							LIMdlLod*        src);

static int private_lod_copy(LIMdlBuilderLod* self,
							LIMdlBuilderLod* src);

static void private_lod_clear(LIMdlBuilderLod* self);

static int private_realloc_array(void*  array,
								 int*   capacity,
								 int    required,
								 size_t size);
 ...
 */
LIMdlBuilder* limdl_builder_new(LIMdlModel* model)
{
	int j;
	LIMdlBuilder* self;
	...
	int           vertex_offset)
{
	...
	for (i = vertex_offset ; i < self->model->vertices.count ; i++)
	{
		...
		value = lialg_memdic_find (lookup, &key, sizeof (vec3));
		if (value == NULL)
		{
			value = lisys_calloc (1, sizeof (vec3));
			if (value != NULL)
			{
				*value = self->model->vertices.array[i].normal;
				if (!lialg_memdic_insert (lookup, &key, sizeof (vec3), value))
					lisys_free (value);
			}
		}
		else
