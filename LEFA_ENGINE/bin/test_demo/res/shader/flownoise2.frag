...
{
    vec2 Ps = P + dot(P, vec2(F2)); // Skew transform
    vec2 Pi = floor(Ps);            // Simplex origin in grid space
    vec2 P0 = Pi - dot(Pi, vec2(G2)); // Origin back in standard space
    vec2 v0 = P - P0; // Relative position to simplex origin
    vec2 i1 = (v0.x > v0.y) ? vec2(1.0, 0.0) : vec2(0.0, 1.0);
    vec2 v1 = v0 - i1 + G2;        // Second corner relative position
    vec2 v2 = v0 - 1.0 + 2.0 * G2; // Third corner relative position
    Pi = mod(Pi, 289.0);           // Wrap at 289
